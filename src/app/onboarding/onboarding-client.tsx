...
export function OnboardingClient() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col items-center justify-start py-20 p-6 sm:p-12 selection:bg-primary/10">
      <div className="max-w-4xl w-full flex flex-col items-center gap-16">
          ...
          {TOPICS.map((topic) => {
            ...
            return (
              <motion.button
                ...
                className={cn(
                  "group relative flex flex-col items-center justify-center gap-5 p-8 aspect-square rounded-3xl border-hairline transition-all duration-500",
                  isSelected
        ...
        <motion.div
          ...
          className="fixed bottom-12 left-0 right-0 flex flex-col items-center px-6 pointer-events-none"
        >
          <div className="sr-only" aria-live="polite">
            {selected.length >= 3 ? "Ready to start. You have selected enough topics." : ""}
          </div>
          <AnimatePresence>
            {selected.length >= 3 && (
              <motion.div
